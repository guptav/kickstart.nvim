...
-- Configure the shell used by Neovim
...
vim.api.nvim_create_user_command('RG', function(args)
  ...
  if not args.fargs or #args.fargs == 0 then
    -- If no arguments are provided, use the current word under the cursor
    vim.cmd 'FzfLua grep_cword'
    return
  end
  local first_arg = args.fargs[1]
  -- Otherwise, use the provided argument as the search term
  vim.cmd('FzfLua grep search=' .. first_arg)
end, { nargs = '*' })
...
require('lazy').setup({
...

  {
    ...
    opts = {
      fzf_tmux_opts = {
        -- Use the following options to set the size of the fzf window
        --  See `:help fzf-tmux` for more information
        ['-p'] = '80%,80%',
      },
    },
  },
...

  {
    ...
    vim.keymap.set('n', '<F3>', "<cmd>VimuxRunCommand ''.getline('.')<CR>j", { nowait = true }),
